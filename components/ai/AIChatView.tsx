...
const AIChatView: React.FC<AIChatViewProps> = ({
  ...
  const initializeChat = useCallback(async () => {
    ...
    try {
      ...
      const topMatchesInfo = careerMatches
        .slice(0, 3)
        .map(
          (match) =>
  ...
  const handleSendMessage = async () => {
    if (!userInput.trim() || !chatSession || isLoading || turnsLeft === 0)
      return;
  ...
  return (
    <div
      className="mt-8 sm:mt-10 bg-white p-4 sm:p-6 rounded-xl shadow-xl"
      data-oid="evbkpoy"
    >
      <h2
        className="text-xl sm:text-2xl font-semibold text-slate-800 mb-4 text-center"
        data-oid="op.n2d6"
      >
        AIキャリア相談
      </h2>
      ...
      {isLoading && messages.length === 0 && !error && (
        <div className="text-center py-5" data-oid="yh2x1io">
          <div
            className="animate-spin rounded-full h-8 w-8 border-t-2 border-b-2 border-blue-500 mx-auto mb-3"
            data-oid="rwo12a1"
          ></div>
          <p className="text-slate-600" data-oid="u6.7g07">
            AIチャットを準備中です...
          </p>
        </div>
      )}
      ...
      <div
        className="h-72 sm:h-96 overflow-y-auto border border-slate-200 rounded-lg p-3 sm:p-4 mb-4 space-y-3 bg-slate-50"
        data-oid="g6a122e"
      >
        {messages.map((msg) => (
          <div
          ...
          >
            ...
            >
              {msg.text.split("\n").map((line, index) => (
                <span key={index} data-oid="nlc2e8e">
                  {line}
                  <br data-oid="7:27g_w" />
                </span>
              ))}
        ...
        <div ref={messagesEndRef} data-oid="r.lsb40" />
      </div>

      {turnsLeft > 0 ? (
        <div className="flex items-center gap-2 sm:gap-3" data-oid="809:tnb">
          <input
            type="text"
            value={userInput}
        ...
        >
          <p
            className="text-slate-700 mb-3 text-sm sm:text-base"
            data-oid="qxk4vep"
          >
