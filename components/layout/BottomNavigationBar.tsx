      ...
      <div className="max-w-screen-sm mx-auto flex justify-around items-stretch h-16 sm:h-18">
        {NAV_ITEMS.map((item) => {
          ...
          return (
            ...
            >
              <item.Icon className={`w-6 h-6 mb-0.5 transition-colors duration-200 ${isActive ? 'stroke-blue-600' : 'stroke-slate-500'}`} />
              <span className={`text-xs leading-tight transition-all duration-200 ${isActive ? 'font-semibold text-blue-600' : 'font-normal'}`}>
                {item.label}
              </span>
            </Link>
          );
